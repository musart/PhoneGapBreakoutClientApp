...
BO.WSocketWrapper = (function() {
	...
	WSocketWrapper.prototype.init = function(self) {
		...
		if (self._useSocketIO) {
			...
			try {
				/** @private */
				self._socket.on('connect', function() { console.log("init() connect");
					self.dispatchEvent(new WSocketEvent(WSocketEvent.CONNECTED));
					/** @private */
					self._socket.on('message', function(msg) { console.log("init() message");
			...
			try {
				...
				console.log("wait onopen()");
				/** @private */
				self._socket.onopen = function() {
					console.log("onopen");
					self.dispatchEvent(new WSocketEvent(WSocketEvent.CONNECTED));
					/** @private */
					self._socket.onmessage = function(msg) { console.log("onmessage");
	...
	WSocketWrapper.prototype.send = function(message) {
	    console.log("WSocketWrapper.send()");
